...
uint8_t button_getButton() {
	...
	if((GPIO_PORTE_DATA_R & 0b00001000) == 0b00000000){  //check if first bit of data is low or high. Return 4 if low (pressed)
	    cyBot_sendByte('m');
	    return '4';
	        }

	if((GPIO_PORTE_DATA_R & 0b00000100) == 0b00000000){  //check if first bit of data is low or high. Return 3 if low (pressed)
                return '3';
            }

    if((GPIO_PORTE_DATA_R & 0b00000010) == 0b00000000){  //check if first bit of data is low or high. Return 2 if low (pressed)
                return '2';
            }

    if((GPIO_PORTE_DATA_R & 0b00000001) == 0b00000000){  //check if first bit of data is low or high. Return 1 if low (pressed)
                return '1';
            }
